...
import { generateBloodAnalysisPDF, generateSymptomCheckerPDF, generateXRayPDF } from "@/lib/pdf-generator"
...
export default function Profile() {
  ...
  const handleDownloadPDF = (scan: any) => {
    const result = scan.result || scanAnalyses[scan.id]
    if (!result) return

    if (scan.type === 'blood') {
        generateBloodAnalysisPDF(result)
    } else if (scan.type === 'symptom' || !scan.type) {
        generateSymptomCheckerPDF({ ...result, symptoms: scan.symptoms })
    } else if (scan.type === 'x-ray') {
  ...
  const toggleScanExpansion = async (scanId: string, symptoms: string[], scanType: string = "symptom", scanResult?: any) => {
    ...
    if (!isExpanded && scanResult && Object.keys(scanResult).length > 0) {
    ...
    if (!isExpanded && !scanAnalyses[scanId] && !analyzingScans[scanId] && scanType === 'symptom') {
                      ...
                      <CardContent className="p-4">
                        <div className="flex items-start justify-between mb-3">
                          ...
                          <div className="flex items-center gap-2">
                            {(scan.type === 'blood' || scan.type === 'x-ray' || scan.type === 'symptom' || !scan.type) && (
                                <Button
                                    variant="outline"
                                    size="sm"
                            ...
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => toggleScanExpansion(scan.id, scan.symptoms, scan.type, scan.result)}
                              className="flex items-center gap-2"
                            >
                        ...
                        <div className="space-y-4">
                          ...
                          {expandedScans[scan.id] && (
                            <div className="mt-4 pt-4 border-t">
                              {scan.type === 'blood' && scan.result ? (
                                <div className="space-y-4">
                                    <div className="bg-primary/5 rounded-lg p-4">
                                        <div className="flex items-center gap-2 mb-2">
                                            <Brain className="h-4 w-4 text-primary" />
                                            <h4 className="font-semibold">Blood Analysis Results</h4>
                                        </div>
                                        <p className="text-sm text-muted-foreground">{scan.result.irregularity_info}</p>
                                    </div>
                                    {scan.result.irregularities && scan.result.irregularities.length > 0 && (
                                        <div className="bg-muted/30 rounded-lg p-3">
                                            <h5 className="font-medium text-sm mb-2">Irregularities</h5>
                                            <ul className="text-sm list-disc ml-5">
                                                {scan.result.irregularities.map((item: string, i: number) => (
                                                    <li key={i}>{item}</li>
                                                ))}
                                            </ul>
                                        </div>
                                    )}
                                    {scan.result.possible_diseases && scan.result.possible_diseases.length > 0 && (
                                        <div className="bg-muted/30 rounded-lg p-3">
                                            <h5 className="font-medium text-sm mb-2">Possible Diseases</h5>
                                            <ul className="text-sm list-disc ml-5">
                                                {scan.result.possible_diseases.map((item: string, i: number) => (
                                                    <li key={i}>{item}</li>
                                                ))}
                                            </ul>
                                        </div>
                              ...
                              ) : scan.type === 'x-ray' && scan.result ? (
                                <div className="space-y-4">
                                    <div className="bg-primary/5 rounded-lg p-4">
                                        <div className="flex items-center gap-2 mb-2">
                                            <Activity className="h-4 w-4 text-primary" />
                                            <h4 className="font-semibold">X-Ray Analysis Results</h4>
                                        </div>
                                        <p className="text-lg font-medium">{scan.result.study_type}</p>
                                        {scan.result.summary && <p className="text-sm text-muted-foreground mt-2">{scan.result.summary}</p>}
                                    </div>
                                    {scan.result.key_findings && scan.result.key_findings.length > 0 && (
                                        <div className="bg-muted/30 rounded-lg p-3">
                                            <h5 className="font-medium text-sm mb-2">Key Findings</h5>
                                            <ul className="text-sm list-disc ml-5">
                                                {scan.result.key_findings.map((item: string, i: number) => (
                                                    <li key={i}>{item}</li>
                                                ))}
                                            </ul>
                                        </div>
                                    )}
                                    {scan.result.likely_conditions && scan.result.likely_conditions.length > 0 && (
                                        <div className="bg-muted/30 rounded-lg p-3">
                                            <h5 className="font-medium text-sm mb-2">Likely Conditions</h5>
                                            <ul className="text-sm list-disc ml-5">
                                                {scan.result.likely_conditions.map((item: string, i: number) => (
                                                    <li key={i}>{item}</li>
                                                ))}
                                            </ul>
                                        </div>
                                    )}
                                    {scan.result.top_finding && scan.result.top_finding.details && (
                                        <div className="bg-primary/5 rounded-lg p-3 border border-primary/20">
                                            <h5 className="font-bold text-sm mb-2 text-primary">Top Finding: {scan.result.top_finding.name}</h5>
                                            <p className="text-xs text-muted-foreground mb-2">{scan.result.top_finding.details.short_description}</p>
                                            <div className="grid grid-cols-2 gap-2 text-[10px]">
                                                <div>
                                                    <p className="font-semibold">Symptoms:</p>
                                                    <ul className="list-disc ml-3">
                                                        {scan.result.top_finding.details.common_symptoms?.slice(0, 3).map((s: string, i: number) => <li key={i}>{s}</li>)}
                                                    </ul>
                                                </div>
                                                <div>
                                                    <p className="font-semibold">Prevention:</p>
                                                    <ul className="list-disc ml-3">
                                                        {scan.result.top_finding.details.precautions_and_prevention?.slice(0, 3).map((s: string, i: number) => <li key={i}>{s}</li>)}
                                                    </ul>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    {scan.result.recommendations && scan.result.recommendations.length > 0 && (
                                        <div className="bg-muted/30 rounded-lg p-3">
                                            ...
                                            <ul className="text-sm list-disc ml-5">
                                                {scan.result.recommendations.map((item: string, i: number) => (
                                                    <li key={i}>{item}</li>
                                                ))}
                                            </ul>
                                        </div>
