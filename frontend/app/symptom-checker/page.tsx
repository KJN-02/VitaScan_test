...
import { generateSymptomCheckerPDF } from "@/lib/pdf-generator"
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Fetch detailed disease info automatically
        try {
          const infoRes = await fetch("/api/disease-info", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ disease: result.primary_prediction }),
          })
          const infoData = await infoRes.json()
          
          let completeResult = result;
          if (infoData.success) {
            setDiseaseInfo(infoData.info)
            completeResult = { ...result, disease_info: infoData.info }
          }

          // Save scan to user's history if logged in
          if (user) {
            await addScan(selectedSymptoms, "symptom", completeResult)
          }
        } catch (infoError) {
          console.error("Failed to fetch disease info:", infoError)
          // Still save the basic result if detailed info fails
          if (user) {
            await addScan(selectedSymptoms, "symptom", result)
          }
        }
                ...
                {analysisResult.success ? (
                  <>
                    <div className="text-center">
                      ...
                      <Button variant="outline" size="sm" onClick={handleDownloadPDF} className="mb-4 bg-transparent border-white text-white hover:bg-white hover:text-black">
                      ...
                      <div className="flex flex-col items-center gap-3">
                        {infoError && (
                          <div className="text-center">
                            ...
                            {infoDetail && (
                              <p className="text-xs text-gray-400 mt-1">{infoDetail}</p>
                            )}
                          </div>
                        )}
                        {diseaseInfo && (
                          <div className="text-left bg-gray-800/50 rounded p-4 max-w-2xl space-y-3">
                            {diseaseInfo.short_description && (
                              <p className="text-sm text-gray-200">{diseaseInfo.short_description}</p>
                            )}
                            {Array.isArray(diseaseInfo.common_symptoms) && diseaseInfo.common_symptoms.length > 0 && (
                              <div>
                                <p className="text-sm font-medium text-white">Common Symptoms</p>
                                <ul className="text-sm text-gray-300 list-disc ml-5">
                                  {diseaseInfo.common_symptoms.map((s: string, i: number) => (
                                    <li key={i}>{s}</li>
                                  ))}
                                </ul>
                              </div>
                            )}
                            {Array.isArray(diseaseInfo.common_causes_or_risk_factors) &&
                              diseaseInfo.common_causes_or_risk_factors.length > 0 && (
                                <div>
                                  <p className="text-sm font-medium text-white">Causes / Risk Factors</p>
                                  <ul className="text-sm text-gray-300 list-disc ml-5">
                                    {diseaseInfo.common_causes_or_risk_factors.map((s: string, i: number) => (
                                      <li key={i}>{s}</li>
                                    ))}
                                  </ul>
                                </div>
                              )}
                            {Array.isArray(diseaseInfo.precautions_and_prevention) &&
                              diseaseInfo.precautions_and_prevention.length > 0 && (
                                <div>
                                  <p className="text-sm font-medium text-white">Precautions & Prevention</p>
                                  <ul className="text-sm text-gray-300 list-disc ml-5">
                                    {diseaseInfo.precautions_and_prevention.map((s: string, i: number) => (
                                      <li key={i}>{s}</li>
                                    ))}
                                  </ul>
                                </div>
                              )}
                            {diseaseInfo.when_to_see_a_doctor && (
                              <div>
                                <p className="text-sm font-medium text-white">When to See a Doctor</p>
                                <p className="text-sm text-gray-300">{diseaseInfo.when_to_see_a_doctor}</p>
                              </div>
                            )}
                            {diseaseInfo.confidence && (
                              <p className="text-xs text-gray-400">Confidence: {diseaseInfo.confidence}</p>
                            )}
                          </div>
                        )}
                      </div>
                    ...
                    {diseaseInfo && analysisResult.recommendations && (
                      <div>
